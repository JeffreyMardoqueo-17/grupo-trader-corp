import type { SocialLinksData } from "./team-data";

interface LandingAdvisorFooterProps {
  name: string;
  role: string;
  socialLinks: SocialLinksData;
}

export function LandingAdvisorFooter({ name, role, socialLinks }: LandingAdvisorFooterProps) {
  return (
    <footer className="relative w-full overflow-hidden border-t border-white/10 bg-[#0c1526]">
      <div className="mx-auto max-w-7xl px-6 py-12">
        <div className="flex flex-col items-center gap-8">
          ...
          <div className="text-center text-sm text-white/40">
            <p className="mb-1">{name} &middot; {role}</p>
            <p>&copy; {new Date().getFullYear()} Grupo Trade Corp. Todos los derechos reservados.</p>
          </div>
        </div>
      </div>
    </footer>
